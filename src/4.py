...
OUTPUT_DIR = "highlight_v4"

# ⚠️ 실제 사용자의 GCS 버킷 이름으로 변경하세요!
GCS_BUCKET_NAME = "shortsgenie-voice" 

# 💡 STT에 넘길 최대 클립 개수 설정 (이 숫자가 낮을수록 속도가 빨라집니다)
MAX_CLIPS_FOR_STT = 100
...
def detect_highlight_candidates_vad(audio_path, model, get_speech_timestamps, read_audio, 
                                    vad_threshold=0.95, min_clip_duration=3.0): 
    ...
    # 💡 임계값 0.95로 상향 및 짧은 침묵(200ms)은 연결하여 속도 최적화
    ...
    print(f"⚽ 탐지된 후보 구간 수: {len(candidates)}개 (VAD 임계값: {vad_threshold})\n")
    return candidates
...
def extract_highlight_clips(audio_path, candidates):
    print(f"✂️ [3/5] 후보 구간 오디오 추출 중 (총 {len(candidates)}개 클립)...")
...
def transcribe_candidates_async(clip_paths, candidates, bucket_name):
    print("🗣️ [4/5] 비동기 STT 변환 중 (다중 클립 동시 병렬 처리)...")
    ...
    for i, (operation, (gcs_uri, clip_start_original)) in enumerate(zip(operations, gcs_files_info)):
        ...
        try:
            # 병렬 처리된 결과가 도착할 때까지 대기 (타임아웃 설정)
            response = operation.result(timeout=1000) 
        except Exception as e:
            print(f"   ❌ STT 작업 실패 ({os.path.basename(gcs_uri)}): {e}")
            continue

        # 3. 결과 파싱 및 원본 시간 오프셋 적용 (타임라인 확보)
...
if __name__ == "__main__":
    ...
    if "GOOGLE_APPLICATION_CREDENTIALS" not in os.environ:
        ...
        if vad_model is None:
            ...
            # VAD 극단적 최적화 설정으로 후보 탐지
            candidates = detect_highlight_candidates_vad(
                cleaned, 
                vad_model, 
                get_speech_timestamps, 
                read_audio, 
                vad_threshold=0.95, # 💡 속도 최적화
